...
## Nav publique, accueil et protection des CRUD (27/08/2026)
- **Nav publique épurée** : le menu principal ne contient que du contenu de visiteur (Accueil, Spectacles, Artistes, Critiques, Critiques presse, Contact, À propos). Types/Localités/Lieux/Représentations sont des tables de gestion, retirées du menu public (déjà accessibles via le dashboard admin). Mes réservations/Espace producteur/Tableau de bord/Administration déplacés dans le bloc utilisateur à droite.
- **Page d'accueil = catalogue** : le PID exige explicitement que le catalogue des spectacles (avec lieu et prochaines représentations) soit intégré à l'accueil, avec un moteur de recherche/tri/filtres. `_shows_catalog_context()` (`reservations/views.py`) est désormais partagé entre `index()` et `show_index()` : les deux pages ont la même recherche par titre, filtres (lieu, réservable), tri et pagination.
- **Bug de sécurité corrigé (audit CRUD)** : les vues `_create`/`_edit` du catalogue (Artist, Type, Locality, Location, Show, Representation) ne vérifiaient que `login_required`, jamais la permission Django — un simple membre (groupe MEMBER, view_* uniquement par design) pouvait créer/modifier ces données via l'URL. Ajout de `permission_required('reservations.add_X'/'change_X')` sur les 12 vues concernées, symétrique à ce qui existait déjà sur `delete`. Templates : les liens Ajouter/Modifier/Supprimer, jusque-là affichés sans condition (visibles même par un visiteur anonyme), sont maintenant gardés par `{% if perms.reservations.X %}`.
...
## Demande/approbation de rôle + workflow de proposition de spectacle (27/08/2026)
Devenir CRITIC ou PRODUCER se faisait jusque-là uniquement à la main via Django Admin. L'audit demandé par l'utilisateur a aussi révélé deux failles de sécurité réelles, corrigées dans la foulée :
- `/producteur/` n'avait **aucune** protection par rôle (juste `@login_required`) — n'importe quel membre connecté pouvait y accéder.
- `review_validate` ne vérifiait que la permission Django globale `change_review` (que PRODUCER et ADMIN possèdent tous les deux) sans jamais vérifier que la critique concernait un spectacle produit par l'appelant — contrairement à `press_article_publish`, qui faisait déjà cette vérification. Un producteur pouvait donc valider la critique de n'importe quel spectacle.

Implémenté :
- **Modèle `RoleRequest`** (user, role CRITIC/PRODUCER, message optionnel, status pending/approved/rejected, reviewed_by, reviewed_at). Vue `role_request_create` (libre-service, bloque les doublons), `role_request_approve`/`reject` (admin uniquement) — approuver ajoute directement au groupe Django correspondant. Section « Rôles » sur la page profil (liens conditionnels + historique de mes demandes), section « Demandes de rôle en attente » sur le tableau de bord admin.
- **Protection `/producteur/`** : `@user_passes_test(is_producer)` (nouvelle fonction `is_producer()`, groupe PRODUCER uniquement — **l'ADMIN n'y a plus accès automatique**, décision confirmée par l'utilisateur). Nouveau `reservations/context_processors.py` (`role_flags`) expose `user_is_admin`/`user_is_producer` à tous les templates ; le lien nav « Espace producteur » utilise désormais `user_is_producer`.
- **Fix sécurité `review_validate`** : ajout de la même garde de propriété que `press_article_publish` (`show.producer_id == request.user.id or is_admin(request.user)`).
- **`Show.published`** (BooleanField, default=True pour ne rien casser sur les spectacles existants) : un producteur peut désormais **proposer** un nouveau spectacle (`show_propose`, réservé au groupe PRODUCER, `ShowProposalForm` sans champ producer/published) qui reste invisible du catalogue public (`published=False`) jusqu'à validation par un admin (`show_publish`, `permission_required('change_show')`). `_shows_catalog_context` (accueil + `/spectacles/`) et `show_show` filtrent désormais sur `published=True` (sauf prévisualisation par le producteur ou un admin). Une fois publié, le producteur ne modère (`review_validate`, `press_article_publish`) que les critiques/articles de ses propres spectacles publiés — cohérent avec le PID (« modérer les commentaires et critiques de presse de ces spectacles »).
...
- **Bugs UX rapportés et corrigés** : la page profil n'avait qu'un lien ambigu « Poster une critique » menant vers le commentaire membre (Review), pas d'article de presse — un critique approuvé ne trouvait donc aucun moyen de soumettre un article. Séparé en deux sections claires avec liens distincts, tous deux masqués si non applicables (pas de réservation pour le commentaire, pas de permission pour l'article).
- **Bug de permissions corrigé (2e occurrence du même piège que la migration 0010)** : la migration 0014 assignait les permissions PressArticle à CRITIC/PRODUCER dans le **même** `migrate` que la 0013 qui crée le modèle PressArticle — ces permissions n'existaient pas encore en base à ce moment-là (le signal `post_migrate` qui les crée ne se déclenche qu'à la fin de toute la commande `migrate`). Résultat vérifié en base : CRITIC avait 0 permission PressArticle, PRODUCER n'avait pas `change_pressarticle`, MEMBER n'était plus à jour non plus. Migration `0018` resynchronise les 4 groupes dans son propre `migrate`. **Règle à ne plus jamais casser** : une migration de données qui assigne des permissions à un groupe doit toujours dépendre d'une migration antérieure déjà appliquée dans un `migrate` précédent, jamais de la migration qui crée le modèle dans le même batch — sinon resynchroniser séparément après coup, comme ici.
